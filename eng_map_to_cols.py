...
# "D:\download\res_eng_chinese_map (3).json"
import json_util
import string_util
...
def file_json_to_under_score_lower_map(filepath= rf"D:\download\res_eng_chinese_map (3).json"):

    eng_map=json_util.file_path_to_dict(filepath= filepath)
    for i in eng_map:
        # eng_map[i]=eng_map[i].lower()
        eng_map[i]=string_util.eng_to_underscore_lower(eng_map[i])

    print(eng_map)

    json_util.json_to_file(eng_map,filename= rf"D:\proj\job\res_eng_chinese_map_underscore_lower.json")
