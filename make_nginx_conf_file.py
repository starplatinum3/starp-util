...
# root_path=f'/home/app/zucc-web'

# root_path=f'/home/work/app/exam-sys-db/src/main/resources/static/admin'
root_path=f'/home/work/app/exam-sys-db/src/main/resources/static/student'
# student
...
out_path=f"{nginx_root_dir}/nginx_{port}_{now_time_str}.conf"
nginx_conf_path=out_path
# 这里要写入 但是还没有创建目录
...
if doStart:
    ...
    # os.system(f"nginx -c {nginx_root_dir}/nginx_{port}.conf")
    os.system(f"nginx -c {nginx_conf_path}")
    # nginx_conf_path
    print("查看nginx 有没有启动")
    print(f"sudo lsof -i :{port}")
    os.system(f"sudo lsof -i :{port}")
