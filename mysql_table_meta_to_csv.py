...
def mysql_table_meta_to_csv(do_write=True):
    ...
    # TABLE_COMMENT_set=set(df['TABLE_COMMENT'])
    TABLE_COMMENT_set=set()
    ...
    for TABLE_NAME in TABLE_NAME_set:
        # print(TABLE_NAME)
        if TABLE_NAME in dont_need_table_name_list:
            continue
        ...
        TABLE_COMMENT_set.add(table_comment)
    ...
    # print(TABLE_NAME_says)
    TABLE_COMMENT_set_says="表、".join(TABLE_COMMENT_set)
    print("TABLE_COMMENT_set_says")
    print(TABLE_COMMENT_set_says)
    out_doc_file=rf'D:\bishe\tables_doc\tables_doc_{now_time_str}.docx'
    print("out_doc_file")
    print(out_doc_file)
...
def list_camel_to_underscore_lower(lst):
    res=[]
    for i in lst:
        res.append(string_util.camel_to_underscore_lower(i))
    return res

def get_json_key_commnets(table_obj,cols,json):
    pass
    table_name=table_obj['TABLE_NAME']
    TABLE_COMMENT=table_obj['TABLE_COMMENT']
    TABLE_COMMENT_show=get_TABLE_COMMENT_show(table_obj)
    ...
    # print(table_obj)
    json_keys=json.keys()
    # string_util.camel_to_underscore_lower()
    json_keys_under_score=list_camel_to_underscore_lower(json_keys)
    ...
    for col_info in cols:
        ...
        COLUMN_COMMENT_show=COLUMN_COMMENT if COLUMN_COMMENT else COLUMN_NAME
        COLUMN_COMMENT_show_list.append(COLUMN_COMMENT_show)
        if COLUMN_NAME in set(json_keys_under_score):
            # print("COLUMN_COMMENT_show")
            # print(COLUMN_COMMENT_show)
            # print("COLUMN_NAME")
            # print(COLUMN_NAME)
            print(f'{COLUMN_NAME} : {COLUMN_COMMENT_show}')
            # json_keys_translate_list.append({
            #     'COLUMN_NAME':COLUMN_NAME,
            #     'COLUMN_COMMENT_show':COLUMN_COMMENT_show,
            # })
        # if DATA_TYPE:
        # public class Blog {
        #     [Column(IsIdentity = true, IsPrimary = true)]
...
def get_cols_comments_of_json(json):
    # do_write=False
    do_write=True
    connection = pymysql.connect(host=host,
    user=username,
    ...
    """
    {
         'TABLE_NAME': 'all_visit_count',
            'TABLE_COMMENT': '', 
         'TABLE_CATALOG': 'def', 'TABLE_SCHEMA': 'problemsolvingsystem',
     'TABLE_TYPE': 'BASE TABLE',
      'ENGINE': 'InnoDB', 'VERSION': 10, 'ROW_FORMAT': 'Dynamic',
        'TABLE_ROWS': 31, 'AVG_ROW_LENGTH': 528, 'DATA_LENGTH': 16384,
          'MAX_DATA_LENGTH': 0, 'INDEX_LENGTH': 0, 'DATA_FREE': 0, 
          'AUTO_INCREMENT': None, 'CREATE_TIME': datetime.datetime(2022, 10, 30, 12, 46, 56),
            'UPDATE_TIME': datetime.datetime(2022, 12, 13, 10, 3, 26), 'CHECK_TIME': None, 
            'TABLE_COLLATION': 'utf8_general_ci', 'CHECKSUM': None, 'CREATE_OPTIONS': '',
            'MAX_INDEX_LENGTH': 0, 'TEMPORARY': 'N'},
    ...
    table_name_need='t_exam_paper_answer'
    # print(table_name)
    ...
    # res=get_json_key_commnets(table_obj,cols,json)
...
dont_need_table_name_list=['t_question_2','eyesight_res','physical_test','t_job_link',]
...
# physical_test
    # dont_need_table_name_list=['t_question_2','eyesight_res','physical_test','t_job_link',]
    # dont_need_table_name_list=[tables['t_question'],tables['t_question_type'],tables['t_question_type_subject'],tables['t_subject'],tables['t_user']]
    # 试卷列表，可以查看学科、等，可以进行编辑、删除等操作
    out_file_dir=rf"D:\毕设\lunWenDesp"
    # now_time_str=time_util.get_now_time_str()
    # out_file_dir=f"{out_file_dir}/{now_time_str}"
...
if __name__ =="__main__":
    ...
    json_obj= {
                "id": 251,
                "createTime": "2023-04-07 20:02:57",
                "userScore": "0",
                "studentUserId": 6,
                "subjectName": "面经",
                "subjectId": 15,
